...
static t_loc hpc2loc(t_hpc hpc) {
    double tmp, z, s, phi;
    // 2 - (x + y) is the north-south diagonal of the pixel diamond
    // f/4 adds 0, 1, or 2 depending on the row of the pixel
    double jr = 2 - (hpc.x + hpc.y) + hpc.f/4;
    if (jr < 1.) {
        // north polar cap
        tmp = jr*jr*(1./3.);
        z = 1. - tmp;
...
static t_hpd loc2hpd(int64_t nside, t_loc loc) {
    ...
    return (t_hpd){hpc.x*nside, hpc.y*nside, hpc.f};
}


static t_loc hpd2loc(int64_t nside, t_hpd hpd) {
  t_hpc hpc = (t_hpc){(hpd.x + 0.5)/nside, (hpd.y + 0.5)/nside, hpd.f};
  return hpc2loc(hpc);
}
...
int64_t ang2pix(int64_t nside, t_ang ang, bool nest) {
    return nest ? ang2nest(nside, ang) : ang2ring(nside, ang);
}
...
int64_t vec2pix(int64_t nside, t_vec vec, bool nest) {
    return nest ? vec2nest(nside, vec) : vec2ring(nside, vec);
}
...
t_ang pix2ang(int64_t nside, int64_t ipix, bool nest) {
    return nest ? nest2ang(nside, ipix) : ring2ang(nside, ipix);
}
...
t_vec pix2vec(int64_t nside, int64_t ipix, bool nest) {
    return nest ? nest2vec(nside, ipix) : ring2vec(nside, ipix);
}
...
static t_loc randloc(int64_t nside, int64_t ipix, double u1, double u2,
                     bool nest) {
    t_hpd hpd = nest ? nest2hpd(nside, ipix) : ring2hpd(nside, ipix);
    return hpc2loc((t_hpc){(hpd.x + u1)/nside, (hpd.y + u2)/nside, hpd.f});
}


t_ang randang(int64_t nside, int64_t ipix, double u1, double u2, bool nest) {
    return loc2ang(randloc(nside, ipix, u1, u2, nest));
}


t_vec randvec(int64_t nside, int64_t ipix, double u1, double u2, bool nest) {
    return loc2vec(randloc(nside, ipix, u1, u2, nest));
}
